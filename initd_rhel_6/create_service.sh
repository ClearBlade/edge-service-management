...
## Removing Service & fail siltently
...
stop() {
    ...
    rm -f \$lockfile
    [ \$retval -eq 0 ] && success \$"Stopped $service_name successfully...\n"
    return \$retval
}

cb_status() {
  # see if running
  prog=\$(basename \$PROG_BIN)
  #local pids=\$(cat \$PROG_PIDFILE)
  status \$prog
  # if [ -n "\$pids" ]; then
  #   echo -e "\$prog (pid \$pids) is running"
  # else
  #   echo -e "\n \$prog is stopped"
  # fi
  return 0
...
cat >$service_name <<EOF
PROG_BIN=$bin_path
PROG_PIDFILE=/var/run/$service_name.pid
params=$params
EOF
